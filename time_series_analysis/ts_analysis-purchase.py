...
if __name__ == '__main__':
    ...
    ts = pd.read_csv('purchase_metrics.csv', header=0, parse_dates=[0], index_col=[0])
    ts = ts.astype(int)
    print(ts)
    ...
    result = seasonal_decompose(ts, model='mul', period=10)
    result.plot()
    plt.show()
    ...
    # nel nostro caso d = 2
    train = trend.iloc[:334] # 90%
    test = trend.iloc[334:] # 10%
    model = ARIMA(train, order=(10,2,0)) #training --> ARIMA(p,d,q)
    results = model.fit()
    start = len(train)
    ...
    # aumentando di un'unità il valore di d, otteniamo un modello (ARIMA con order = (10,3,0)) che matcha quasi perfettamente la nostra serie

    train = trend.iloc[:334] # 90%
    test = trend.iloc[334:] # 10%
    model = ARIMA(train, order=(10,3,0)) #training
    results = model.fit()
    ...
    model = ARIMA(trend, order=(10,3,0))
    results = model.fit()
