...
if __name__ == '__main__':
    ...
    model = ARIMA(train, order=(15,2,0)) #training --> ARIMA(p,d,q)
    results = model.fit()
    start = len(train)
    ...
    # aumentando di un'unità il valore di d, otteniamo un modello (ARIMA con order = (15,3,0)) che matcha perfettamente la nostra serie

    train = trend.iloc[:334] # 90%
    test = trend.iloc[334:] # 10%
    model = ARIMA(train, order=(15,3,0)) #training
    results = model.fit()
    ...
    results = model.fit()
    fcast = results.predict(len(trend), len(trend)+72, typ='levels') # predizione di 72 campioni = 6 minuti
    plt.figure(figsize=(24,10))
    plt.ylabel('Values', fontsize=14)
    plt.xlabel('Time', fontsize=14)
    plt.title('Time series', fontsize=16)
